...
DATA = json.loads((ROOT / "_rawdata" / "kinder.json").read_text(encoding="utf-8"))
...
for region in sorted(counts):
    ...
    page_title = f"{full} 유치원 찾기"
    title_h1 = f"{full} 유치원 {cnt:,}곳"
    subtitle = f"{full} 지역 국공사립 유치원을 시군구별로 검색하세요."

    content = f"""---
layout: region
title: {page_title}
description: {full} 유치원 {cnt}곳의 위치, 전화번호, 운영시간, 학급수·원아정원을 확인하세요.
do_name: {region}
title_h1: {title_h1}
subtitle: {subtitle}
---
